...
def window(window_size, extract_path, last_participant,dataIS):
    ...
    if dataIS == "EEG":
        ...
        for j in range(5): #int(h.shape[0]/skip_num)
            # train model
            for i in range(len(files)):
                ...
                if this_participant == next_participant: 
                    ...
                    if f'VP0{this_participant}' in files[i] and '0-back' in files[i]:
                        df_0_back = pd.read_csv(files[i], skiprows = j+skip_num, nrows = window_size)
                        ...
                        df_2_back = pd.read_csv(files[i], skiprows = j+skip_num, nrows = window_size)
                        ...
                        df_3_back = pd.read_csv(files[i], skiprows = j+skip_num, nrows = window_size)
            ...
            for i in range(len(files)):
                this_participant = f'{files[i][files[i].find("VP0")+3]}{files[i][files[i].find("VP0")+4]}'
                if this_participant == f'{last_participant}':
                    first_part = i
                    break
            ...
            for i in range(first_part,len(files)):
                ...
                if this_participant == next_participant: 
                ...
                #if next_participant != this_participant and != '' then analyze the data                 
                    ...
                    rf_acc += acc_values[0]
                    svm_acc += acc_values[1]
                    knn_acc += acc_values[2]
                    nn_acc += acc_values[3]
                    gauss_acc += acc_values[4]
                ...
                #last_participant reached
                    ...
                    X = hold.iloc[:,2:].to_numpy()
                    Y = hold.iloc[:,0].to_numpy()
                    acc_val = pickle_model_test(X,Y)

            rf_accuracy.append(rf_acc/c)
            svm_accuracy.append(svm_acc/c)
            knn_accuracy.append(knn_acc/c)
            nn_accuracy.append(nn_acc/c)
            gauss_accuracy.append(gauss_acc/c)
            
            print('rf accuracy: ',rf_acc/c)
            print ('svm accuracy: ',svm_acc/c)
            print ('knn accuracy: ',knn_acc/c)
            print ('nn accuracy: ',nn_acc/c)
            print ('gauss accuracy: ',gauss_acc/c)
