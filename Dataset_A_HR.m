...
for vp = 1 : length(subdir_list)
	...
	subplot(2,2,2);
	plot_channel(epo.deoxy, 'C3h', 'YLim',[-0.006, 0.006]);
	subplot(2,2,3);
    ...
    cd('Figures');
    num = string(vp);
    name = strcat('Participant', num,'.jpg');
    name2 = char(name);
    saveas(gcf,name2);
    %}
...
%% Plot figures
subplot(2,2,1);
plot_channel(epo_all.deoxy, 'AF7', 'YLim',[-0.006, 0.006]);
subplot(2,2,2);
plot_channel(epo_all.deoxy, 'C3h', 'YLim',[-0.006, 0.006]);
subplot(2,2,3);
plot_channel(epo_all.oxy, 'AF7', 'YLim',[-0.006, 0.006]);
subplot(2,2,4);
plot_channel(epo_all.oxy, 'C3h', 'YLim',[-0.006, 0.006]);

%% Save Figure
cd(WorkingDir);
cd('Figures');
saveas(gcf,'OverAll.jpg');
